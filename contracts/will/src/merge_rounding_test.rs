...
fn merge_beneficiaries_preserves_small_share() {
    ...
    let will_b_tokens: SorobanVec<(Address, i128)> = vec![&env, (token_address.clone(), 9_900_000i128)];
    ...
    assert!(small_balance > 0, "beneficiary_small should receive a share (not be dropped)");
    assert!(large_balance > 0, "beneficiary_large should receive a share");
    assert_eq!(total, 10_000_000, "total distributed should equal combined will balance");
}
...
fn merge_multiple_small_shares_preserved() {
    ...
    let will_a_beneficiaries: SorobanVec<Beneficiary> = vec![
        &env,
        Beneficiary { address: small_1.clone(), allocation: Allocation::Percentage(100) },
        Beneficiary { address: small_2.clone(), allocation: Allocation::Percentage(100) },
        Beneficiary { address: small_3.clone(), allocation: Allocation::Percentage(100) },
        Beneficiary { address: large.clone(), allocation: Allocation::Percentage(9_700) },
    ];
    let will_a_tokens: SorobanVec<(Address, i128)> = vec![&env, (token_address.clone(), 100_000i128)];
    ...
    assert_eq!(total, 10_000_000, "total should equal combined will balance");
}
