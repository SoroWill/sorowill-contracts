...
fn test_will_created_event_snapshot() {
    ...
    assert!(data.3 > env.ledger().timestamp(), "checkin_deadline should be in future");
}
...
fn test_inheritance_released_event_snapshot() {
    ...
    assert_eq!(data.1, beneficiaries_count, "event beneficiaries_count mismatch");
}
...
fn test_beneficiaries_updated_event_snapshot() {
    ...
    let data: (Address, u32, soroban_sdk::Vec<Beneficiary>) = 
        event_data.try_into_val(&env).unwrap();
    assert_eq!(data.0, owner, "event owner mismatch");
    assert_eq!(data.1, beneficiary_count, "event beneficiary_count mismatch");
    assert_eq!(data.2, beneficiaries, "event beneficiaries mismatch");
}
...
fn test_batch_and_advanced_events_snapshot() {
    ...
    assert_eq!(data.1, new_checkin_period_days, "periods_updated event checkin_period mismatch");
    assert_eq!(data.2, new_grace_period_days, "periods_updated event grace_period mismatch");
    assert_eq!(data.3, next_deadline, "periods_updated event next_deadline mismatch");
}
...
fn test_final_events_snapshot() {
    ...
    let data: (Address, soroban_sdk::Vec<soroban_sdk::Symbol>) = event_data.try_into_val(&env).unwrap();
    assert_eq!(data.0, owner, "settings_updated event owner mismatch");
    assert_eq!(data.1, update_fields, "settings_updated event update_fields mismatch");
